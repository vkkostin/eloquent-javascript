...
myModules.myExports.elementFromChar = function(legend, ch) {
	if (ch === " ")
		return null;
...
myModules.myExports.charFromElement = function(element) {
	if (element === null)
		return " ";
	else
	...
	map.forEach(function(line, y) {
		for (let x = 0; x < line.length; x++)
			grid.set(new Vector(x, y),
				myModules.myExports.elementFromChar(legend, line[x]));
	});
}
...
World.prototype.toString = function() {
	...
	for (let y = 0; y < this.grid.height; y++) {
		for (let x = 0; x < this.grid.width; x++) {
			...
			output += myModules.myExports.charFromElement(element);
		}
		output += "\n";
	}
	return output;
...
View.prototype.look = function(dir) {
	...
	if (this.world.grid.isInside(target))
		return myModules.myExports.charFromElement(this.world.grid.get(target));
	else
		return "#";
};
...
actionTypes.reproduce = function(critter, vector, action) {
	const baby = myModules.myExports.elementFromChar(this.legend,
		critter.originChar);
	const dest = this.checkDestination(action, vector);
	if (dest === null ||
